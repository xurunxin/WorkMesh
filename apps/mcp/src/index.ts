...
function registerCoordinationTools(server: McpServer, client: WorkMeshClient, mode: McpMode | undefined): void {
  ...
  server.registerTool('list_claimable_work_items', { description: 'List non-terminal Issues this Connection can atomically claim: unassigned Issues plus an exact same-identity assignment when every non-terminal execution Session is stale; live work:read and work:write authorization is revalidated. Pass nextCursor back as cursor.', inputSchema: { cursor: z.string().max(8192).optional(), limit: z.number().int().min(1).max(200).optional() } }, async input => tool(() => client.listClaimableWorkItems({ cursor: input.cursor, limit: input.limit })))
