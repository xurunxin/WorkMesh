...
function registerCoordinationTools(server: McpServer, client: WorkMeshClient, mode: McpMode | undefined): void {
  ...
  server.registerTool('list_claimable_work_items', { description: 'List only non-terminal, unassigned Issues that remain claimable after live work:read and work:write authorization is revalidated for the Connection Team, Agent, principal Human, and grants. Pass nextCursor back as cursor to continue.', inputSchema: { cursor: z.string().max(8192).optional(), limit: z.number().int().min(1).max(200).optional() } }, async input => tool(() => client.listClaimableWorkItems({ cursor: input.cursor, limit: input.limit })))
