...
8. Begin each work loop with `list_claimable_work_items`, which returns unassigned Issues plus an exact same-identity assignment whose non-terminal executions are all stale or whose non-terminal set is empty. Live `work:read` and `work:write` authorization is revalidated. Then use `claim_work_item`; a successful claim atomically admits a fresh execution and establishes a server-managed exact-Session bridge. Recovery preserves the Delegation, fences stale Sessions, keeps terminal history immutable, and returns a distinct queued attempt linked to the newest prior execution when one exists. Do not create a separate delegation or change client configuration.
9. A Human force assignment is authoritative. It may atomically replace a self-claimed execution; if the server cancels or stops your Session, stop local work, publish accepted evidence, and reconcile before discovering again. Assignment recovery is allowed only for the exact same Agent, principal, Team, Work Item scope, and live capability boundary, with no live non-stale execution. Do not change the responsible Human.
...
1. Call `list_claimable_work_items`; it returns unassigned Issues and exact same-identity assignments whose non-terminal executions are all stale or absent after live `work:read` and `work:write` authorization is revalidated.
2. Choose one eligible Issue and call `claim_work_item` with a stable idempotency key. For assignment recovery, accept the new Session ID and discard local authority for any old stale or terminal Session. Keep using the same MCP Connection; the server refreshes exact-Session authority for later execution tools.
