...
test.describe('Task 6.6 phone Chinese keyboard and semantic journey', () => {
  ...
  test('covers compact selectors, mobile navigation, local table scrolling, top layers, and containment', async ({ page }, testInfo) => {
    ...
    const settingsTabs = page.locator('.settings-page > .wm-tabs')
    await expectCompactTabs(settingsTabs, '390 Settings tabs')
    const settingsSelector = settingsTabs.locator(':scope > .wm-tab-list-compact select.wm-tab-select')
    expect.soft(await tabUntilFocused(page, settingsSelector), '390 Settings selector is keyboard reachable').toBe(true)
    if (await settingsSelector.evaluate(element => element === document.activeElement)) await page.keyboard.press('End')
    await expect.soft(page.locator('.settings-tab-heading > h2'), '390 embedded Operations remains h2').toHaveCount(1)
    await expectSemanticIntegrity(page, '390 Embedded Operations panel')
