...
async function installMcpFixture(page: Page, supportedClients: readonly string[] = ['opencode', 'generic_mcp']): Promise<RequestAudit> {
  ...
  await page.route(`${apiUrl}/**`, async route => {
    ...
    if (path === '/mcp') {
      ...
      return body({ error: { code: 'UNAUTHORIZED', message: 'credential required', correlationId: 'mocked-onboarding-e2e' } }, 401)
    }
    return body({ error: { code: 'NOT_FOUND', message: 'Unexpected mocked onboarding route.', correlationId: 'mocked-onboarding-e2e' } }, 404)
  })
  return audit
}
...
test('uses only advertised clients and keeps keyboard, copy, and request boundaries local', async ({ page }, testInfo) => {
  ...
  await expectNoForbiddenClientWasMounted(page)
  await attachJson(testInfo, 'task-5.5-mocked-interaction-request-evidence', {
...
test('fails closed when discovery advertises no usable client', async ({ page }) => {
  ...
  await expect(page.locator('[role="alert"][data-onboarding-state="unsupported_client"]')).toContainText('Unsupported client')
...
for (const viewport of viewportCases) {
  test(`keeps deliberate onboarding geometry at ${viewport.name}`, async ({ context, page }, testInfo) => {
    ...
    console.info('TASK_5_5_MOCKED_GEOMETRY', JSON.stringify({ name: viewport.name, geometry }))

    const screenshotPath = testInfo.outputPath(`task-5.5-mocked-connect-${viewport.name}.png`)
    await page.screenshot({ path: screenshotPath, fullPage: true })
    await testInfo.attach(`task-5.5-mocked-connect-${viewport.name}`, { path: screenshotPath, contentType: 'image/png' })
    await attachJson(testInfo, `task-5.5-mocked-connect-${viewport.name}-geometry`, {
...
test('keeps untrusted public discovery detail out of the document', async ({ page }) => {
  await page.route(`${apiUrl}/**`, route => route.fulfill({
    status: 503,
    headers,
    body: JSON.stringify({ error: { code: 'SERVICE_UNAVAILABLE', message: 'untrusted-upstream-detail', correlationId: 'mocked-onboarding-e2e' } }),
  }))
  await page.goto('/connect#test')
  ...
  await expect(page.locator('body')).not.toContainText('untrusted-upstream-detail')
})
