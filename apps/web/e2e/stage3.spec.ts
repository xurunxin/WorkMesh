...
test('renders the Stage 3 fake-provider delivery shelf without auto-closing work', async ({ page }) => {
  ...
  await page.route(`${apiUrl}/api/v1/**`, async route => {
    ...
    if (path === '/api/v1/projects') return body({ items: [project], nextCursor: null })
    ...
    if (path === '/api/v1/views') return body({ items: [], nextCursor: null })
    if (path === '/api/v1/teams/team-1/states') return body({ items: [{ id: 'state-started', name: 'In progress', category: 'started', color: '#2563eb', revision: 1 }], nextCursor: null })
