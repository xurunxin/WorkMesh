...
test('Recovery Center preserves evidence, governed actions, canonical state, and responsive layout', async ({ page, context }, testInfo) => {
  ...
  const evidenceTrigger = center.getByRole('button', { name: /Commit abc123/ })
  ...
  await expect(page).toHaveURL(new RegExp('evidenceId='))
  await expect(page.getByRole('dialog', { name: 'Commit abc123' })).toBeVisible()
  await page.goBack()
