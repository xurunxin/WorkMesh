...
test('renders a real API-backed multi-agent Work Room and controls durable collaboration state', async ({ page }) => {
  ...
  const created = await humanApi<{ session: { id: string; revision: number } }>(page, `/api/v1/work-items/${work.body.id}/agent-session`, 'POST', { agentId: rootAgent.body.id, principalHumanActorId: me.body.actor.id, role: 'executor', requestedCapabilities: ['work:read', 'work:write', 'plan:write', 'artifact:write'], initialPrompt: 'Coordinate the Stage 2 browser acceptance flow.', budget: { maxRuntimeSeconds: 600 } }, work.body.revision)
  expect(created.status).toBeLessThan(300)
  const refreshed = await agentApi<{ sessionToken: string }>(page, rootAgent.body.installation_token, `/api/v1/agent-sessions/${created.body.session.id}/token/refresh`, 'POST', {})
  expect(refreshed.status).toBeLessThan(300)
  const acknowledged = await agentApi<{ revision: number }>(page, refreshed.body.sessionToken, `/api/v1/agent-sessions/${created.body.session.id}/ack`, 'POST', { summary: 'Accepted the Stage 2 browser flow.', externalUrls: [] })
  expect(acknowledged.status).toBeLessThan(300)
  const executing = await agentApi<{ revision: number }>(page, refreshed.body.sessionToken, `/api/v1/agent-sessions/${created.body.session.id}/state`, 'POST', { state: 'executing', reason: 'Running the Stage 2 browser flow.' }, acknowledged.body.revision)
  ...
  const contextArtifact = await agentApi<{ id: string }>(page, refreshed.body.sessionToken, '/api/v1/artifacts', 'POST', { sessionId: created.body.session.id, workItemId: work.body.id, type: 'test_report', title: 'Stage 2 context evidence', uri: 'https://evidence.example.test/stage2-context-report', checksum: evidenceHash, metadata: {} })
  const delta = await agentApi(page, refreshed.body.sessionToken, `/api/v1/agent-sessions/${created.body.session.id}/context-deltas`, 'POST', { baseSnapshotId: context.body.contextSnapshotId, additions: [{ sourceType: 'artifact', sourceId: contextArtifact.body.id, hash: evidenceHash }], rationale: 'Added real API-backed Stage 2 evidence.' })
  ...
  const handoff = await agentApi<{ id: string }>(page, refreshed.body.sessionToken, '/api/v1/handoffs', 'POST', { fromSessionId: created.body.session.id, targetAgentId: childAgent.body.id, summary: 'Review the Stage 2 evidence.', completedWork: ['Created collaboration room'], remainingWork: ['Validate audit cards'], openQuestions: [], risks: [], acceptanceCriteria: ['Human can inspect messages'], requestedAction: 'Review and report', leaseTransferPolicy: 'retain', artifactIds: [], requestedCapabilities: ['work:read', 'work:write'] })
  const decision = await agentApi<{ id: string }>(page, refreshed.body.sessionToken, `/api/v1/agent-sessions/${created.body.session.id}/decisions`, 'POST', { title: 'Ship Stage 2 collaboration UI?', rationale: 'The durable room shows human-visible messages and controls.', options: ['ship', 'hold'], evidence: [], affectedResources: [] })
