...
test('manages versioned Guidance while keeping Project description separate', async ({ page }) => {
  ...
  await page.getByLabel('起始指南版本').selectOption(revision1.id)
  await page.getByLabel('目标指南版本').selectOption(revision2.id)
  ...
  await page.getByPlaceholder('归档或回滚时必填').fill('Retire current instructions')
  await page.getByRole('button', { name: '归档当前指南' }).click()
  ...
  await page.getByLabel('指南作用域').selectOption('project')
  await expect(page.getByText('项目描述（不属于指南）')).toBeVisible()
