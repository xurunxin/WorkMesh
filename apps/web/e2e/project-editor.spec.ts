...
test('Project creation and revisioned edit keep Markdown and the selected URL after reload', async ({ page }) => {
  ...
  let detail = page.locator('.work-item-detail-sheet')
  ...
  const relatedIssueTitle = `Related issue ${Date.now()}`
  await page.locator('.project-detail-pane').getByRole('button', { name: /新建 Issue|New issue/ }).click()
  const relatedCreate = page.getByTestId('create-work-item')
  await relatedCreate.locator('input[name="title"]').fill(relatedIssueTitle)
  await relatedCreate.getByTestId('create-work-item-submit').click()
  await expect(relatedCreate).toBeHidden()
  await firstIssue.click()
  detail = page.locator('.work-item-detail-sheet')
  await detail.getByRole('tab', { name: /概览|Overview/ }).click()
  const relationships = detail.locator('.relationship-panel')
  const target = relationships.locator('select[name="targetWorkItemId"] option').filter({ hasText: relatedIssueTitle })
  await expect(target).toHaveCount(1)
  await relationships.locator('select[name="targetWorkItemId"]').selectOption(await target.getAttribute('value') ?? '')
  await relationships.getByRole('button', { name: /添加关系|Add relationship/ }).click()
  await expect(relationships.locator('.relation-list')).toContainText(relatedIssueTitle)
  await detail.getByRole('button', { name: /打开完整页面|Open full page/ }).click()
