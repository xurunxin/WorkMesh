...
test('an administrator configures and revokes a model service without exposing its credential', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /停用模型|Disable model/ }).click()
  await expect(page.getByRole('status')).toContainText(/模型已停用|Model disabled/)
  await expect(page.getByRole('button', { name: /启用模型|Enable model/ })).toBeVisible()

  await page.reload()
  await expect(page.getByText(/MiniMax M3/)).toBeVisible()
  await expect(page.getByRole('button', { name: /启用模型|Enable model/ })).toBeVisible()
  await page.getByRole('button', { name: /启用模型|Enable model/ }).click()
  await expect(page.getByRole('status')).toContainText(/模型已启用|Model enabled/)
  await expect(page.locator('body')).not.toContainText(secret)
