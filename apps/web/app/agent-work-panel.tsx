...
import { type Agent, type AgentSession, activeAgentTeamAccess, agentName, agentProvider, agentStateClass, agentStateLabel, approvedAgentCapabilitiesForTeam, canPauseAgentSession, canRetryAgentSession, canStopAgentSession, delegateAndStart, formatTime, optionalAgentRequest, retryAgentSession } from './lib/agents'
...
export function AgentWorkPanel({ workItemId, workItemTeamId, workItemRevision, humanActorId, onSessionCreated }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      const [nextAgents, nextSessions] = await Promise.all([
        optionalAgentRequest<Agent[]>('/api/v1/agents'),
        optionalAgentRequest<AgentSession[]>(`/api/v1/agent-sessions?workItemId=${encodeURIComponent(workItemId)}`),
      ])
      setAgents(nextAgents ?? [])
      setSessions(nextSessions ?? [])
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to load agent status.') }
  }, [workItemId])
  ...
  const signal = async (session: AgentSession, signalName: 'pause' | 'resume' | 'stop') => {
    try {
      ...
      const updated = await apiRequest<AgentSession>(`/api/v1/agent-sessions/${session.id}/signals`, {
  ...
  const delegate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSessions(current => [session, ...current]); setShowDelegate(false); onSessionCreated?.(session)
