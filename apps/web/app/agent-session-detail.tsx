...
import { type AgentActivity, type AgentSession, type Approval, type ApprovalDecision, type Artifact, type PlanVersion, agentStateClass, agentStateLabel, canPauseAgentSession, canRetryAgentSession, canStopAgentSession, classifyApprovalDecisionFailure, decideApproval, formatApprovalPayload, formatTime, normalizeActivity, normalizeApproval, normalizeArtifact, normalizePlan } from './lib/agents'
...
import { ApprovalDecisionControls, type ApprovalDecisionUiState } from './agents/approval-decision-controls'
...
export function AgentSessionDetail({ sessionId, compact = false, tab }: Props) {
  ...
  const decide = async (approval: Approval, decision: ApprovalDecision, reason?: string): Promise<boolean> => {
    if (approvalDecisionStates[approval.id]?.status === 'busy' || approvalDecisionStates[approval.id]?.status === 'success') return false
    ...
    try {
      ...
      window.setTimeout(() => void approvalsPage.refresh(), 1200)
      return true
    } catch (reason) {
      ...
      if (failure === 'conflict' || failure === 'expired' || failure === 'authority_inactive') void approvalsPage.refresh()
      return false
    }
  }
  ...
  return <section className={compact ? 'agent-session-detail compact' : 'agent-session-detail'} data-testid="agent-session-detail">
    ...
    {session && <><dl className="session-facts"><div><dt>{text.factState}</dt><dd>{agentStateLabel(session.state)}</dd></div><div><dt>{text.factPrincipal}</dt><dd>{humans.find(human => human.id === session.principal_human_actor_id)?.display_name ?? (session.principal_human_actor_id ? session.principal_human_actor_id.slice(0, 8) : text.notReported)}</dd></div><div><dt>{text.factSession}</dt><dd>{session.id.slice(0, 8)}</dd></div><div><dt>{text.factCurrentStep}</dt><dd>{selectedPlan?.steps.find(step => step.status === 'in_progress')?.title ?? text.notReported}</dd></div><div><dt>{text.factHeartbeat}</dt><dd>{formatTime(session.last_heartbeat_at)}</dd></div><div><dt>{text.factBudget}</dt><dd>{session.budget.maxRuntimeSeconds ? text.maxRuntimeSeconds(session.budget.maxRuntimeSeconds) : text.defaultPolicy}</dd></div></dl>
      ...
      <section className="approval-inbox" aria-label={text.approvalInboxTitle}><h3>{text.approvalInboxTitle}</h3>{pendingApprovals.length === 0 ? <p className="empty">{text.approvalInboxEmpty}</p> : pendingApprovals.map(approval => <article key={approval.id}><strong>{approval.action_name}</strong><span className={`risk-${approval.risk_level}`}>{text.approvalRisk(approval.risk_level)}</span><p>{approval.rationale_summary}</p><details className="approval-scope-details" data-testid={`session-approval-scope-${approval.id}`}><summary>{agentsCopy.approvalPayloadLabel}</summary><pre className="approval-payload" data-testid={`session-approval-payload-${approval.id}`} style={{ maxWidth: '100%', margin: 0, overflowX: 'auto', overflowWrap: 'anywhere', whiteSpace: 'pre-wrap' }}>{formatApprovalPayload(approval.action_payload_sanitized)}</pre>{approval.action_payload_hash && <code className="approval-payload-hash">{approval.action_payload_hash}</code>}</details><nav aria-label={agentsCopy.approvalContextLabel} className="approval-context-links"><a href={`/agent-sessions/${sessionId}`}>{agentsCopy.sessionLabel?.(sessionId.slice(0, 8)) ?? `Session ${sessionId.slice(0, 8)}`}</a>{session.work_item_id && <a href={`/?workItemId=${encodeURIComponent(session.work_item_id)}`}>{agentsCopy.workItemLabel?.(session.work_item_id.slice(0, 8)) ?? `Work item ${session.work_item_id.slice(0, 8)}`}</a>}<a href={`/agent-sessions/${sessionId}?tab=artifacts`}>{agentsCopy.approvalEvidenceLink}</a></nav>{approval.decisions?.filter(decision => decision.source === 'human').map((decision, index) => <p className="approval-history-decision" data-testid={`session-approval-decision-${approval.id}-${index}`} key={`${decision.actor_id}-${decision.decided_at}`}><strong>{decision.decision}</strong><span className="approval-decision-reason">{decision.reason}</span></p>)}<small>{text.approvalExpires(formatTime(approval.expires_at))}</small><ApprovalDecisionControls approval={approval} copy={agentsCopy} onDecide={decide} state={approvalDecisionStates[approval.id]} /></article>)}<LoadMoreButton collection={approvalsPage} label={text.loadMoreApprovals} /></section></>}
      {(!tab || tab === 'plan') && <section className="plan-panel" aria-label={text.planVersionsTitle}><header><h3>{text.planVersionsTitle}</h3><label>{text.planCurrent}<select aria-label={text.planCurrent} value={selectedPlan?.id ?? ''} onChange={event => setSelectedPlanId(event.currentTarget.value)}>{plans.map(plan => <option key={plan.id} value={plan.id}>v{plan.revision} · {plan.change_summary}</option>)}</select></label></header>{selectedPlan ? <><ol>{selectedPlan.steps.slice().sort((a, b) => a.ordinal - b.ordinal).map(step => <li key={step.id}><strong>{step.title}</strong><span className={`plan-step step-${step.status}`}>{text.planStepStatus(step.status)}</span>{step.description && <RichContent density="compact" source={step.description} />}{step.acceptanceCriteria.length > 0 && <small>{text.acceptanceCriteria(step.acceptanceCriteria.join('; '))}</small>}</li>)}</ol>{plans.length > 1 && <section className="plan-compare" aria-label={text.planVersionsTitle}><label>{text.planCompareWith}<select aria-label={text.planCompareWith} value={comparisonPlan?.id ?? ''} onChange={event => setComparePlanId(event.currentTarget.value)}>{plans.filter(plan => plan.id !== selectedPlan.id).map(plan => <option key={plan.id} value={plan.id}>v{plan.revision} · {plan.change_summary}</option>)}</select></label>{comparisonPlan && <ul>{stepChanges.length === 0 ? <li>{text.planStepChangedTitle(comparisonPlan.revision, selectedPlan.revision)}</li> : stepChanges.map(change => <li key={change.current?.id ?? change.previous?.id}>{!change.previous ? <strong>{text.planAdded(change.current?.title ?? '')}</strong> : !change.current ? <strong>{text.planRemoved(change.previous.title)}</strong> : <><strong>{text.planChanged(change.previous.title, change.current.status)}</strong>{change.previous.title !== change.current.title && <> · {text.planRenamed(change.previous.title, change.current.title)}</>}</>}</li>)}</ul>}</section>}</> : <p className="empty">{text.planNoPlan}</p>}<LoadMoreButton collection={plansPage} label={text.loadMorePlans} /></section>}
      {(!tab || tab === 'activity') && <section className="activity-panel" aria-label={text.activityTitle}><header><h3>{text.activityTitle}</h3><label className="heartbeat-toggle"><input type="checkbox" checked={showHeartbeats} onChange={event => setShowHeartbeats(event.currentTarget.checked)} /> {text.showHeartbeats}</label></header><div className="activity-filters" role="group" aria-label={text.activityTitle}>{([{ value: 'all' as const, label: text.activityFilterAll }, { value: 'actions' as const, label: text.activityFilterActions }, { value: 'questions' as const, label: text.activityFilterQuestions }, { value: 'evidence' as const, label: text.activityFilterEvidence }, { value: 'errors' as const, label: text.activityFilterErrors }]).map(item => <button aria-pressed={filter === item.value} className={filter === item.value ? 'selected' : ''} key={item.value} onClick={() => setFilter(item.value)} type="button">{item.label}</button>)}</div>{visibleActivities.length === 0 ? <p className="empty">{text.activityEmpty}</p> : <ol className="activity-timeline">{visibleActivities.map(activity => <li key={activity.id}><div><strong>{text.activityKind(activity.kind)}</strong><time>{formatTime(activity.created_at)}</time></div><RichContent density="compact" source={activity.detailsMarkdown ?? activity.summary} />{activity.toolInvocation && <small>{text.activityTool(activity.toolInvocation.toolName, activity.toolInvocation.status)}</small>}</li>)}</ol>}<LoadMoreButton collection={activitiesPage} label={text.loadMoreActivities} /></section>}
