...
export function AgentRunTimeline({ compact = false, onControl, sessionId }: Props) {
  ...
  const drawerItems = useMemo<EvidenceDrawerItem[]>(() => explanation?.evidenceDetails.map(item => {
    const group = explanation.causalGroups.find(candidate => candidate.evidence.some(reference => reference.id === item.id))
    const step = explanation.planVersions.flatMap(version => version.steps.map(candidate => ({ ...candidate, versionId: version.id }))).find(candidate => candidate.id === item.planStepId)
    const technical = group?.technicalRecords.find(record => record.references.some(reference => reference.type === 'artifact' && reference.id === item.id))
    return {
      ...item,
      freshness: explanation.freshness.state,
      producer: group ? { id: group.actor.id, label: group.actor.displayName, kind: group.actor.kind } : undefined,
      principalHuman: explanation.responsibleHuman ? { id: explanation.responsibleHuman.id, label: explanation.responsibleHuman.displayName } : undefined,
      sessionId,
      workItem: explanation.workItem ? { id: explanation.workItem.id, label: explanation.workItem.title, projectId: explanation.project?.id } : undefined,
      plan: item.planStepId ? { versionId: step?.versionId, stepId: item.planStepId, stepLabel: step?.title } : undefined,
      action: group ? { id: group.id, label: group.summary, correlationId: technical?.correlationId ?? undefined } : undefined,
      validation: { label: group?.validation.summary ?? item.validationState, exactHeadSha: item.repository?.commit ?? undefined },
      repository: item.repository ?? undefined,
    }
  }) ?? [], [explanation, sessionId])
  const evidenceDrawer = useEvidenceDrawer(drawerItems, `run:${sessionId}`)
  ...
  return <section aria-busy={refreshing || undefined} className={`agent-run-timeline ${compact ? 'compact' : 'full'}`} data-testid="run-timeline">
    ...
    <section aria-label={text.evidenceTitle} className="run-evidence-section"><h3>{text.evidenceTitle}</h3>{drawerItems.length === 0 ? <p className="empty">{text.notVerified}</p> : <ul>{drawerItems.map(item => <li id={`evidence-${item.id}`} key={item.id}><span className={`verification verification-${item.validationState}`}>{item.validationState}</span><button onClick={event => evidenceDrawer.open(item, event.currentTarget)} type="button"><strong>{item.title ?? item.id}</strong><small>{item.type}{item.checksum ? ` · ${item.checksum}` : ''}{item.repository?.commit ? ` · ${item.repository.commit}` : ''}</small></button></li>)}</ul>}</section>
