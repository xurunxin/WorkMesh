...
function WorkItemDrawer({ item, humanActorId, states, humans, projects, comments, onClose, onSave, onComment, onUpdateComment }: { item: WorkItem; humanActorId: string; states: WorkflowState[]; humans: Human[]; projects: Project[]; comments: Comment[]; onClose: () => void; onSave: (event: FormEvent<HTMLFormElement>) => Promise<void>; onComment: (event: FormEvent<HTMLFormElement>, parentCommentId?: string) => Promise<void>; onUpdateComment: (comment: Comment, patch: Record<string, string | boolean>) => Promise<void> }) {
  const humanName = (id: string) => humans.find(human => human.id === id)?.display_name ?? id
  const [tab, setTab] = useState<'conversation' | 'plan' | 'activity' | 'artifacts'>('conversation')
  ...
  return <aside className="drawer" aria-label="Work item details" data-testid="work-item-drawer"><header><h2>{item.team_key}-{item.number}</h2><button onClick={onClose}>Close</button></header><form onSubmit={event => void onSave(event)}><label>Title<input name="title" defaultValue={item.title} required /></label><label>Description<textarea name="description" defaultValue={item.description ?? ''} /></label><div className="drawer-grid"><label>Status<select name="statusId" defaultValue={item.status_id}>{states.map(state => <option key={state.id} value={state.id}>{state.name}</option>)}</select></label><label>Priority<select name="priority" defaultValue={item.priority}>{['none', 'urgent', 'high', 'medium', 'low'].map(priority => <option key={priority} value={priority}>{priority}</option>)}</select></label><label>Due date<input name="dueDate" type="date" defaultValue={dateValue(item.due_date)} /></label><label>Owner<select name="ownerId" defaultValue={item.responsible_human_actor_id ?? ''}><option value="">Unassigned</option>{humans.map(human => <option key={human.id} value={human.id}>{human.display_name}</option>)}</select></label><label>Project<select name="projectId" defaultValue={item.project_id ?? ''}><option value="">No project</option>{projects.map(project => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><label>Labels<input name="labels" defaultValue={item.labels.join(', ')} /></label></div><button data-testid="save-work-item">Save changes</button></form>
    <section className="work-tabs" aria-label="Work item agent details"><div role="tablist" aria-label="Work item details tabs">{(['conversation', 'plan', 'activity', 'artifacts'] as const).map(value => <button key={value} role="tab" aria-selected={tab === value} className={tab === value ? 'selected' : ''} onClick={() => setTab(value)}>{value.charAt(0).toUpperCase() + value.slice(1)}</button>)}</div>{tab === 'conversation' && <section className="comments"><h3>Conversation</h3><form onSubmit={event => void onComment(event)}><textarea name="body" placeholder="Write a comment" required /><MentionPicker humans={humans} /><button data-testid="create-comment">Comment</button></form>{comments.length === 0 && <p className="empty">No comments yet.</p>}{comments.map(comment => <article key={comment.id} data-testid={`comment-${comment.id}`}><header><strong>{comment.author_name}</strong><small>{comment.is_resolved ? 'Resolved' : 'Open'}</small></header><p>{comment.body}</p>{comment.mentions.length > 0 && <p className="mention-list">Mentioned: {comment.mentions.map(humanName).map(name => `@${name}`).join(', ')}</p>}<div className="comment-actions"><button onClick={() => { const body = window.prompt('Edit comment', comment.body); if (body?.trim()) void onUpdateComment(comment, { body: body.trim() }) }}>Edit</button><button onClick={() => void onUpdateComment(comment, { isResolved: !comment.is_resolved })}>{comment.is_resolved ? 'Reopen' : 'Resolve'}</button><button onClick={() => { if (window.confirm('Soft-delete this comment?')) void onUpdateComment(comment, { deleted: true }) }}>Delete</button></div><form className="reply-form" onSubmit={event => void onComment(event, comment.parent_comment_id ?? comment.id)}><textarea name="body" placeholder="Reply" required /><MentionPicker humans={humans} /><button>Reply</button></form></article>)}</section>}{tab !== 'conversation' && (activeSession ? <AgentSessionDetail sessionId={activeSession.id} compact tab={tab} /> : <p className="empty">Delegate an agent to show {tab}.</p>)}</section>
