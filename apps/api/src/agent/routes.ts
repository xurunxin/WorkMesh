...
async function readableSession(request: FastifyRequest, h: Helpers, sessionId: string) {
  ...
  const resourceInScope = session.work_item_id
    ...
    : !session.project_id || Boolean(scope.projectIds?.includes(session.project_id));
...
export function registerAgentRoutes(app: FastifyInstance, h: Helpers): void {
  ...
  app.get("/api/v1/agent-sessions/:id/context", async request => { const sessionId = id(request); await readableSession(request,h,sessionId); const session=(await h.db.query("SELECT session.*,CASE WHEN session.work_item_id IS NOT NULL THEN scope_item.project_id ELSE session.project_id END AS effective_project_id FROM agent_sessions session LEFT JOIN work_items scope_item ON scope_item.id=session.work_item_id AND scope_item.workspace_id=session.workspace_id AND scope_item.deleted_at IS NULL WHERE session.id=$1 AND session.workspace_id=$2",[sessionId,actor(request).workspaceId])).rows[0] as {team_id:string;work_item_id:string|null;project_id:string|null;effective_project_id:string|null;current_plan_version_id:string|null;context_snapshot_id:string|null}|undefined; if(!session) throw new DomainError("NOT_FOUND","Agent session not found"); const workItem=session.work_item_id ? (await h.db.query("SELECT w.*,t.key AS team_key,s.name AS status_name,s.category AS status_category FROM work_items w JOIN teams t ON t.id=w.team_id JOIN workflow_states s ON s.id=w.status_id WHERE w.id=$1 AND w.workspace_id=$2 AND w.deleted_at IS NULL",[session.work_item_id,actor(request).workspaceId])).rows[0] ?? null:null; const plan=session.current_plan_version_id ? (await h.db.query("SELECT * FROM agent_plan_versions WHERE id=$1",[session.current_plan_version_id])).rows[0] ?? null:null; const planWithSteps=plan?{...plan as object,steps:(await h.db.query("SELECT * FROM agent_plan_steps WHERE plan_version_id=$1 ORDER BY ordinal",[session.current_plan_version_id])).rows}:null; const guidanceUris=[`workmesh://workspace/${actor(request).workspaceId}/guidance`,`workmesh://team/${session.team_id}/guidance`,...(session.effective_project_id?[`workmesh://project/${session.effective_project_id}/guidance`]:[])]; return {session,workItem,plan:planWithSteps,contextSnapshotId:session.context_snapshot_id,guidanceUris}; });
