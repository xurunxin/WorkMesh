...
async function readableSession(request: FastifyRequest, h: Helpers, sessionId: string) {
  ...
  const subject = session.work_item_id ?? session.project_id;
  if (!session.agent_active || session.delegation_status !== "active" || !session.permissions_snapshot.includes("work:read") || !session.definition_capabilities.includes("work:read") || !session.team_capabilities?.includes("work:read") || !scope.teamIds?.includes(session.team_id) || (subject && !scope.workItemIds?.includes(subject) && !scope.projectIds?.includes(subject))) throw new DomainError("RESOURCE_SCOPE_DENIED", "Agent token lacks read scope for this session");
  return session;
}
...
export function registerAgentRoutes(app: FastifyInstance, h: Helpers): void {
  ...
  app.get("/api/v1/agent-sessions/:id/context", async request => { const sessionId = id(request); await readableSession(request,h,sessionId); const session=(await h.db.query("SELECT * FROM agent_sessions WHERE id=$1 AND workspace_id=$2",[sessionId,actor(request).workspaceId])).rows[0] as {team_id:string;work_item_id:string|null;project_id:string|null;current_plan_version_id:string|null;context_snapshot_id:string|null}|undefined; if(!session) throw new DomainError("NOT_FOUND","Agent session not found"); const workItem=session.work_item_id ? (await h.db.query("SELECT w.*,t.key AS team_key,s.name AS status_name,s.category AS status_category FROM work_items w JOIN teams t ON t.id=w.team_id JOIN workflow_states s ON s.id=w.status_id WHERE w.id=$1",[session.work_item_id])).rows[0] ?? null:null; const plan=session.current_plan_version_id ? (await h.db.query("SELECT * FROM agent_plan_versions WHERE id=$1",[session.current_plan_version_id])).rows[0] ?? null:null; const planWithSteps=plan?{...plan as object,steps:(await h.db.query("SELECT * FROM agent_plan_steps WHERE plan_version_id=$1 ORDER BY ordinal",[session.current_plan_version_id])).rows}:null; const guidanceUris=[`workmesh://workspace/${actor(request).workspaceId}/guidance`,`workmesh://team/${session.team_id}/guidance`,...(session.project_id?[`workmesh://project/${session.project_id}/guidance`]:[])]; return {session,workItem,plan:planWithSteps,contextSnapshotId:session.context_snapshot_id,guidanceUris}; });
