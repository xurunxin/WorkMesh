...
describe('shared Agent mutation resource liveness', () => {
  ...
  it('locks the credential and live Session resources in the shared order', async () => {
    ...
    const sessionLock = command.indexOf('FROM agent_sessions', sessionLocator + 1)
    const credentialLock = command.indexOf('await assertCurrentAgentCredentialInTx')
    const workItemLock = command.indexOf('FROM work_items', credentialLock)
    const projectLock = command.indexOf('FROM projects', workItemLock)

    expect(start).toBeGreaterThanOrEqual(0)
    expect(sessionLock).toBeGreaterThan(sessionLocator)
    expect(credentialLock).toBeGreaterThan(sessionLock)
    expect(workItemLock).toBeGreaterThan(credentialLock)
    expect(projectLock).toBeGreaterThan(workItemLock)
