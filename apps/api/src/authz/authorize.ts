...
const resourceSegments: Readonly<Record<string, {
  ...
  documents: { kind: 'document', resolver: 'document' },
...
function resourceTeamSql(kind: ExplicitResourceKind): string {
  switch (kind) {
    ...
    case 'document': return 'SELECT d.team_id FROM documents d LEFT JOIN projects p ON p.id=d.project_id AND p.workspace_id=d.workspace_id LEFT JOIN work_items w ON w.id=d.work_item_id AND w.workspace_id=d.workspace_id WHERE d.id=$1 AND d.workspace_id=$2 AND (p.id IS NULL OR p.deleted_at IS NULL) AND (w.id IS NULL OR w.deleted_at IS NULL)'
