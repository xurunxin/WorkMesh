...
describe('EventAudiencePolicy SQL', () => {
  ...
  it('never grants Agents blanket Workspace or same-Team visibility', () => {
    ...
    expect(query.sql).toContain('LEFT JOIN projects root_scope_project')
    expect(query.sql).toContain('root_scope_project.deleted_at IS NULL')
    expect(query.sql).toContain('LEFT JOIN projects root_session_project')
    expect(query.sql).toContain('root_session_project.deleted_at IS NULL')
    expect(query.sql).toContain('LEFT JOIN projects child_scope_project')
    expect(query.sql).toContain('child_scope_project.deleted_at IS NULL')
    expect(query.sql).toContain('LEFT JOIN projects child_session_project')
    expect(query.sql).toContain('child_session_project.deleted_at IS NULL')
    expect(query.sql).toContain('WHEN root.work_item_id IS NOT NULL')
    expect(query.sql).toContain('THEN root_scope_project.id')
    expect(query.sql).toContain('root_scope_item.id IS NOT NULL')
    expect(query.sql).toContain('root_session_project.id IS NOT NULL')
    expect(query.sql).toContain('child_scope_item.id IS NOT NULL')
    expect(query.sql).toContain('child_session_project.id IS NOT NULL')
