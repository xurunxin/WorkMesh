...
import { validateRuntimeEnvironment } from './runtime-secrets.mjs'
...
describe('production runtime secrets', () => {
  ...
  it('validates the PostgreSQL password embedded in DATABASE_URL', () => {
    expect(() =>
  ...
  it('rejects cursor material reused as the DATABASE_URL password', () => {
    const reused = randomBytes(32).toString('base64url')
    expect(() =>
      validateRuntimeEnvironment({
        ...runtimeEnvironment,
        DATABASE_URL: `postgres://workmesh:${reused}@postgres/workmesh`,
...
describe('release and feature configuration', () => {
  ...
  it('strictly parses authentication rate-limit and trusted proxy settings', () => {
    const config = loadConfig({
      ...baseEnvironment,
      AUTH_RATE_LIMIT_REDIS_PREFIX: 'authrl:test:config',
      AUTH_RATE_LIMIT_TRUSTED_PROXY_CIDRS: '127.0.0.1/32, 2001:db8::/32',
      AUTH_RATE_LIMIT_SUBJECT_BURST: '12',
      AUTH_RATE_LIMIT_SUMMARY_INTERVAL_MS: '5000',
    })
    ...
    expect(() =>
      loadConfig({
        ...baseEnvironment,
        AUTH_RATE_LIMIT_TRUSTED_PROXY_CIDRS: 'not-a-network',
      }),
    ).toThrow()
    expect(() =>
      loadConfig({
        ...baseEnvironment,
        AUTH_RATE_LIMIT_REDIS_PREFIX: 'authrl:{unsafe}',
      }),
    ).toThrow()
  })
  ...
  it('fails closed for production bootstrap configuration', () => {
    ...
    expect(
      loadConfig({ ...baseEnvironment, NODE_ENV: 'production' }).WORKMESH_BOOTSTRAP_TOKEN,
    ).toBe(bootstrapToken)
  })
  ...
  it('allows tokenless bootstrap only for an explicit isolated non-production loopback bind', () => {
    ...
    expect(() => loadConfig({ ...baseEnvironment, WORKMESH_BOOTSTRAP_TOKEN: undefined })).toThrow(
      /required unless/,
    )
    expect(() =>
      loadConfig({
  ...
  it('rejects malformed, noncanonical, placeholder, and low-diversity bootstrap credentials without echoing them', () => {
    expect(() =>
      loadConfig({
        ...baseEnvironment,
        WORKMESH_BOOTSTRAP_TOKEN: 'not-base64url',
      }),
    ...
    expect(() =>
      loadConfig({
        ...baseEnvironment,
        WORKMESH_BOOTSTRAP_TOKEN: 'Q0hBTkdFX01FX3BsYWNlaG9sZGVyX2Jvb3RzdHJhcF90b2tlbg',
      }),
    ).toThrow(/placeholder/)
    ...
    expect(() =>
      loadConfig({
        ...baseEnvironment,
        WORKMESH_BOOTSTRAP_TOKEN: lowDiversity,
      }),
    ).toThrow(/low-diversity/)
    try {
      loadConfig({
        ...baseEnvironment,
        WORKMESH_BOOTSTRAP_TOKEN: lowDiversity,
  ...
  it('fails closed and rejects unsafe pagination cursor key rings', () => {
    ...
    expect(() =>
      loadConfig({
        ...baseEnvironment,
        DATABASE_URL: `postgres://workmesh:${reused}@localhost/workmesh`,
        PAGINATION_CURSOR_KEYS: `test-1:${reused}`,
      }),
    ).toThrow(/reuse another configured secret/)
    expect(() =>
