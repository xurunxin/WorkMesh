...
export const approvalAutonomyMode = pgEnum('approval_autonomy_mode', ['human_required', 'yolo'])
export const approvalPolicyReconciliationStatus = pgEnum('approval_policy_reconciliation_status', ['pending', 'running', 'completed', 'completed_with_skips'])
export const approvalDecisionSource = pgEnum('approval_decision_source', ['human', 'workspace_policy'])
export const browserPushSubscriptionStatus = pgEnum('browser_push_subscription_status', ['active', 'revoked', 'invalid'])
export const agentEnrollmentPolicyStatus = pgEnum('agent_enrollment_policy_status', ['active', 'revoked'])
...
export const workItemRelationKind = pgEnum('work_item_relation_kind', ['blocks', 'related'])
...
export const workItemRelations = pgTable('work_item_relations', {
  id: uuid('id').primaryKey(), workspaceId: uuid('workspace_id').notNull(), teamId: uuid('team_id').notNull(),
  sourceWorkItemId: uuid('source_work_item_id').notNull(), targetWorkItemId: uuid('target_work_item_id').notNull(), kind: workItemRelationKind('kind').notNull(),
  createdByActorId: uuid('created_by_actor_id'), revision: integer('revision').notNull(), deletedAt: timestamp('deleted_at', { withTimezone: true }),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
...
export const agentDefinitions = pgTable('agent_definitions', {
  ...
  isActive: boolean('is_active').notNull(), archivedAt: timestamp('archived_at', { withTimezone: true }), archivedByActorId: uuid('archived_by_actor_id'), archiveReason: text('archive_reason'), revision: integer('revision').notNull(), createdAt: timestamp('created_at', { withTimezone: true }).notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
...
export const agentConnections = pgTable('agent_connections', {
  ...
  name: text('name').notNull(), agentSlug: text('agent_slug').notNull(), clientType: text('client_type').notNull(), status: text('status').notNull(), source: text('source').notNull(), requestedCapabilities: text('requested_capabilities').array().notNull(), grantedCapabilities: text('granted_capabilities').array().notNull(), grantAgentDelegate: boolean('grant_agent_delegate').notNull(), notes: text('notes'), skillVersion: text('skill_version'), skillSha256: text('skill_sha256'), activeCredentialFingerprintPrefix: text('active_credential_fingerprint_prefix'), pairingCodeExpiresAt: timestamp('pairing_code_expires_at', { withTimezone: true }), lastUsedAt: timestamp('last_used_at', { withTimezone: true }), rotatedAt: timestamp('rotated_at', { withTimezone: true }), revokedAt: timestamp('revoked_at', { withTimezone: true }), enrollmentPolicyId: uuid('enrollment_policy_id'), revision: integer('revision').notNull(), createdByActorId: uuid('created_by_actor_id').notNull(), createdAt: timestamp('created_at', { withTimezone: true }).notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
export const agentEnrollmentPolicies = pgTable('agent_enrollment_policies', {
  id: uuid('id').primaryKey(), workspaceId: uuid('workspace_id').notNull(), teamId: uuid('team_id').notNull(), principalHumanActorId: uuid('principal_human_actor_id').notNull(), name: text('name').notNull(), tokenHash: text('token_hash').notNull(), allowedClientTypes: text('allowed_client_types').array().notNull(), capabilityCeiling: text('capability_ceiling').array().notNull(), grantAgentDelegate: boolean('grant_agent_delegate').notNull(), expiresAt: timestamp('expires_at', { withTimezone: true }).notNull(), maxRedemptions: integer('max_redemptions').notNull(), redemptionCount: integer('redemption_count').notNull(), status: agentEnrollmentPolicyStatus('status').notNull(), revision: integer('revision').notNull(), createdByActorId: uuid('created_by_actor_id').notNull(), revokedAt: timestamp('revoked_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
export const agentEnrollmentRedemptions = pgTable('agent_enrollment_redemptions', {
  id: uuid('id').primaryKey(), policyId: uuid('policy_id').notNull(), connectionId: uuid('connection_id').notNull(), agentId: uuid('agent_id').notNull(), requestFingerprint: text('request_fingerprint').notNull(), createdAt: timestamp('created_at', { withTimezone: true }).notNull(),
})
...
export const workItemExecutorRole = pgEnum('work_item_executor_role', ['primary', 'reviewer'])
export const workItemExecutorProjections = pgTable('work_item_executor_projections', {
  workspaceId: uuid('workspace_id').notNull(), workItemId: uuid('work_item_id').notNull(), projectionRole: workItemExecutorRole('projection_role').notNull(),
  agentId: uuid('agent_id').notNull(), agentActorId: uuid('agent_actor_id').notNull(), sessionId: uuid('session_id').notNull(), leaseId: uuid('lease_id').notNull(),
...
export const artifactUploadStatus = pgEnum('artifact_upload_status', ['pending', 'uploaded', 'verified', 'rejected', 'expired', 'canceled'])
...
export const artifactLinks = pgTable('artifact_links', {
  artifactId: uuid('artifact_id').primaryKey(), workspaceId: uuid('workspace_id').notNull(), projectId: uuid('project_id'), workItemId: uuid('work_item_id').notNull(), sessionId: uuid('session_id'), planStepId: uuid('plan_step_id'), repositoryId: uuid('repository_id'), pullRequestId: uuid('pull_request_id'), provenance: jsonb('provenance').notNull(),
})
export const artifactUploadIntents = pgTable('artifact_upload_intents', {
  id: uuid('id').primaryKey(), workspaceId: uuid('workspace_id').notNull(), workItemId: uuid('work_item_id').notNull(), sessionId: uuid('session_id'), projectId: uuid('project_id'), planStepId: uuid('plan_step_id'), repositoryId: uuid('repository_id'), pullRequestId: uuid('pull_request_id'), headSha: text('head_sha'), sourceTool: text('source_tool').notNull(), requestedByActorId: uuid('requested_by_actor_id').notNull(), storageKey: text('storage_key').notNull(), filename: text('filename').notNull(), mimeType: text('mime_type').notNull(), sizeBytes: bigint('size_bytes', { mode: 'number' }).notNull(), expectedChecksum: text('expected_checksum').notNull(), actualChecksum: text('actual_checksum'), artifactId: uuid('artifact_id'), status: artifactUploadStatus('status').notNull(), attemptCount: integer('attempt_count').notNull(), availableAt: timestamp('available_at', { withTimezone: true }).notNull(), claimedAt: timestamp('claimed_at', { withTimezone: true }), claimedBy: text('claimed_by'), lastError: text('last_error'), expiresAt: timestamp('expires_at', { withTimezone: true }).notNull(), verifiedAt: timestamp('verified_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).notNull(),
})
export const projectMilestones = pgTable('project_milestones', {
  id: uuid('id').primaryKey(), workspaceId: uuid('workspace_id').notNull(), projectId: uuid('project_id').notNull(), name: text('name').notNull(), description: text('description'), targetDate: date('target_date'), revision: integer('revision').notNull(), deletedAt: timestamp('deleted_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
...
export const approvalDecisions = pgTable('approval_decisions', {
  id: uuid('id').primaryKey(), approvalId: uuid('approval_id').notNull(), actorId: uuid('actor_id').notNull(), decision: text('decision').notNull(), reason: text('reason').notNull(), source: approvalDecisionSource('source').notNull(), policyWorkspaceId: uuid('policy_workspace_id'), policyRevision: integer('policy_revision'), decidedAt: timestamp('decided_at', { withTimezone: true }).notNull(),
})
export const approvalAutonomyPolicies = pgTable('approval_autonomy_policies', {
  workspaceId: uuid('workspace_id').primaryKey(), mode: approvalAutonomyMode('mode').notNull(), revision: integer('revision').notNull(), updatedByActorId: uuid('updated_by_actor_id').notNull(), createdAt: timestamp('created_at', { withTimezone: true }).notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
export const approvalAutonomyProjectExclusions = pgTable('approval_autonomy_project_exclusions', {
  workspaceId: uuid('workspace_id').notNull(), projectId: uuid('project_id').notNull(), policyRevision: integer('policy_revision').notNull(), createdByActorId: uuid('created_by_actor_id').notNull(), createdAt: timestamp('created_at', { withTimezone: true }).notNull(),
})
export const approvalPolicyReconciliations = pgTable('approval_policy_reconciliations', {
  id: uuid('id').primaryKey(), workspaceId: uuid('workspace_id').notNull(), policyRevision: integer('policy_revision').notNull(), status: approvalPolicyReconciliationStatus('status').notNull(), processedCount: integer('processed_count').notNull(), approvedCount: integer('approved_count').notNull(), skippedCount: integer('skipped_count').notNull(), lastError: text('last_error'), startedAt: timestamp('started_at', { withTimezone: true }), completedAt: timestamp('completed_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
export const approvalPolicyReconciliationItems = pgTable('approval_policy_reconciliation_items', {
  reconciliationId: uuid('reconciliation_id').notNull(), approvalId: uuid('approval_id').notNull(), status: text('status').notNull(), attemptCount: integer('attempt_count').notNull(), lastError: text('last_error'), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
})
