...
export class WorkMeshClient {
  ...
  listDocuments(ownerType: 'project' | 'work_item', ownerId: string, options: PageRequestOptions = {}): Promise<ListResponse<DocumentResponse>> { return this.request('GET', pagedPath('/api/v1/documents', { ownerType, ownerId }, options), undefined, options) }
  createDocument(input: CreateDocumentInput, options: RequestOptions = {}): Promise<DocumentResponse> { return this.request('POST', '/api/v1/documents', input, { ...options, idempotencyKey: options.idempotencyKey ?? randomUUID() }) }
  getDocument(id: string, options?: RequestOptions): Promise<DocumentResponse> { return this.request('GET', `/api/v1/documents/${encodeURIComponent(id)}`, undefined, options) }
  updateDocument(id: string, input: UpdateDocumentInput, options: RequestOptions & { ifMatch: number | string }): Promise<DocumentResponse> { return this.request('PATCH', `/api/v1/documents/${encodeURIComponent(id)}`, input, { ...options, idempotencyKey: options.idempotencyKey ?? randomUUID() }) }
  listDocumentHistory(id: string, options: PageRequestOptions = {}): Promise<DocumentHistoryResponse> { return this.request('GET', pagedPath(`/api/v1/documents/${encodeURIComponent(id)}/history`, {}, options), undefined, options) }
  getDocumentRevision(id: string, revisionId: string, options?: RequestOptions): Promise<DocumentRevision> { return this.request('GET', `/api/v1/documents/${encodeURIComponent(id)}/revisions/${encodeURIComponent(revisionId)}`, undefined, options) }
  diffDocumentRevisions(id: string, fromRevisionId: string, toRevisionId: string, options?: RequestOptions): Promise<DocumentDiffResponse> { const query = new URLSearchParams({ fromRevisionId, toRevisionId }); return this.request('GET', `/api/v1/documents/${encodeURIComponent(id)}/diff?${query}`, undefined, options) }
  archiveDocument(id: string, input: DocumentArchiveInput, options: RequestOptions & { ifMatch: number | string }): Promise<DocumentResponse> { return this.request('POST', `/api/v1/documents/${encodeURIComponent(id)}/archive`, input, { ...options, idempotencyKey: options.idempotencyKey ?? randomUUID() }) }
  unarchiveDocument(id: string, input: DocumentArchiveInput, options: RequestOptions & { ifMatch: number | string }): Promise<DocumentResponse> { return this.request('POST', `/api/v1/documents/${encodeURIComponent(id)}/unarchive`, input, { ...options, idempotencyKey: options.idempotencyKey ?? randomUUID() }) }
  restoreDocumentRevision(id: string, input: RestoreDocumentRevisionInput, options: RequestOptions & { ifMatch: number | string }): Promise<DocumentResponse> { return this.request('POST', `/api/v1/documents/${encodeURIComponent(id)}/restore`, input, { ...options, idempotencyKey: options.idempotencyKey ?? randomUUID() }) }
