...
describe('Stage 1 agent contracts', () => {
  ...
  it('separates approval request, vote, terminal, and expiry event semantics', async () => {
    ...
    expect(approvalEventEnvelopeSchema.parse({ ...envelope, event_type: 'approval.decision.recorded', payload: { approvalId: id, decision: { actor_id: id, decision: 'approved', reason: 'Looks safe', source: 'human', policy_workspace_id: null, policy_revision: null, decided_at: '2026-07-23T00:00:00.000Z' }, quorum, status: 'pending' } }).event_type).toBe('approval.decision.recorded')
