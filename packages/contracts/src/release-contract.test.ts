...
describe('1.0 release contract documentation', () => {
  it('keeps OpenAPI, protocol, README, and environment flags aligned', () => {
    ...
    const releaseOperations = readRoot('docs/operations/releases.md')
    const candidateWorkflow = readRoot('.github/workflows/release-candidate.yml')
    const promotionWorkflow = readRoot('.github/workflows/promote-ga.yml')
    ...
    expect(releaseOperations).toContain('WORKMESH_RELEASE_WEB_API_URL')
    expect(releaseOperations).toContain('PROMOTION_DIGEST_MISMATCH')
    expect(candidateWorkflow).toContain('uses: ./.github/workflows/ci.yml')
    expect(candidateWorkflow).toContain('pnpm audit --audit-level high')
    expect(promotionWorkflow).not.toMatch(/docker\s+(?:build\b|buildx\s+build\b)/i)
    expect(promotionWorkflow).toContain('PROMOTION_DIGEST_MISMATCH')
  })
})
