...
The active workload phase is deliberately fixed at exactly 24 hours. A shorter run cannot
...
At the 24-hour deadline the harness stops creating new activity events but keeps
the heartbeat pump active. It records tagged `drain` samples for at most
`maximumArchiveLatencyMs` while requiring exact same-run generated/archive
counts, a verified-row delta covering every generated event, archive backlog at
or below its captured baseline, and outbox pending at or below its captured
baseline. If this bounded drain does not converge, the run fails; it is never
continued later or satisfied by a historical segment.

Drain samples are excluded from the active-phase cadence completeness check.
Schema v4 reports both `activeSampleCount` and `drainSampleCount`.
...
- database row/byte, domain-event table byte, dead-tuple, Redis length, or
  per-container RSS long-run growth above the recorded per-hour threshold.
  Long-run growth is the OLS trend over the complete ordered sample series.
  Maximum baseline-to-sample rates remain report diagnostics, while the
  absolute CPU/RSS and other maxima still catch unsafe transient spikes. A
  relative `1e-6` comparison epsilon applies only at a numeric threshold
  boundary to absorb millisecond sampling skew; it does not loosen the recorded
  threshold.

The schema-version-4 final report contains the effective thresholds and the
...
slopes, OLS trend growth rates, maximum baseline-to-sample diagnostic rates,
end-state backlog, and each boolean gate. Exact row accounting requires the
database row delta to equal the generated activity count. The report records
only the successful refresh count and maximum refresh latency plus the
expired-before-refresh count.
