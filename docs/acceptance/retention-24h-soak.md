...
The helper creates the state file exclusively with mode `0600` and schema
version 2. It contains the Session ID, installation token, and non-secret
resource IDs. It never persists the exchanged short-lived Session token. Treat
the state file as a credential, do not copy it into the evidence directory,
and delete it under the acceptance environment's secret-cleanup procedure.
...
latency, with `checks.tokenRotationExercised` requiring at least two refreshes.
