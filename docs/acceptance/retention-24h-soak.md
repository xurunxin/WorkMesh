...
Provision a dedicated Workspace, Agent, delegation, and executing Session with
the tracked helper. Keep its state outside the evidence directory:
...
REDIS_URL=redis://...
WORKMESH_RETENTION_SOAK_API_URL=http://127.0.0.1:3001
WORKMESH_RETENTION_SOAK_SESSION_ID=<sessionId from private provision state>
WORKMESH_RETENTION_SOAK_INSTALLATION_TOKEN=<installationToken from private provision state>
WORKMESH_RETENTION_SOAK_SAMPLE_SECONDS=60
WORKMESH_RETENTION_SOAK_CONTAINERS=<api-container>,<worker-container>,<postgres-container>,<redis-container>,<minio-container>
```

Run exactly one harness process per dedicated Session. Starting the harness
again is a new run: use a new timestamped report directory and capture a new
baseline. The harness creates `samples.jsonl` exclusively and refuses to append
to an earlier run.
...
produce a passing formal report. Formal sample cadence is at most 240 seconds,
which reaches the three-minute proactive refresh margin before a 15-minute
token expires.
...
capped at 60 seconds and total retry delay at 120 seconds. A workload HTTP 401
is terminal and never triggers a reactive refresh. Rotated Session tokens stay
in memory and never enter state, samples, reports, or logs.
...
The schema-version-3 final report contains the effective thresholds, baseline
counters, deltas,
