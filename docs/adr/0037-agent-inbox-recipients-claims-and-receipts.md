...
Migration `0032_agent_inbox_receipts.sql` backfills the existing Human
...
Migration `0033_inbox_receipt_reply_binding.sql` strengthens reply receipts
without editing `0032`. Migration
`0034_legacy_inbox_scope_derivation.sql` backfills Human Inbox rows written by
old producers after `0032` and upgrades the rolling-deploy compatibility
...
`0032` and `0033` migrations remain unchanged. Migration
`0035_decision_session_provenance.sql` binds Agent-created decisions to the
exact authenticated Session.

Spec changes

`OPENAPI.yaml`, `AGENT_PROTOCOL.md`, `SCHEMA.sql`, shared contracts,
route-policy artifacts, Agent SDK, MCP tools, Worker projections, and Stage 2
integration/E2E coverage define the executable boundary.
