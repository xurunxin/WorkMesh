...
RUN pnpm --filter @workmesh/agent-runner... build \
    && pnpm --filter @workmesh/agent-runner --prod deploy /out \
...
ENV NODE_ENV=production WORKMESH_SERVICE=agent-runner WORKMESH_BUILD_SHA=$WORKMESH_BUILD_SHA
LABEL org.opencontainers.image.revision=$WORKMESH_BUILD_SHA \
      org.opencontainers.image.title="WorkMesh Agent Runner"
USER 10001:10001
STOPSIGNAL SIGTERM
ENTRYPOINT ["/app/entrypoint.sh"]
CMD ["node", "dist/run-session.js"]
