...
function UserEnv([string]$Name) {
  ...
  if ($allowed -cnotcontains $Name) { throw "DOGFOOD_V31_CORS_ENV_NAME_REJECTED:$Name" }
  $value = [Environment]::GetEnvironmentVariable($Name,'User')
  if ([string]::IsNullOrWhiteSpace($value)) { throw "DOGFOOD_V31_CORS_ENV_MISSING:$Name" }
  $value
}
...
if ((Get-V31OriginFullPath $ExecutionBindingPath) -cne (Get-V31OriginFullPath $expectedBinding)) { throw 'DOGFOOD_V31_CORS_BINDING_PATH_REJECTED' }
if ((Get-V31OriginSha256 $ExecutionBindingPath) -cne $ExecutionBindingSha256) { throw 'DOGFOOD_V31_CORS_BINDING_SHA_MISMATCH' }
...
if ($binding.kind -ne 'DogfoodV31PublicMcpOriginCorsDecouplingExecutionBinding' -or $binding.selectorBinding -ne $contract.selectorBinding -or $binding.contractSha256 -cne $ContractSha256 -or [string]::IsNullOrWhiteSpace([string]$binding.approvalSha256) -or @('start-api-only.ps1','rollback-api-only.ps1') -cnotcontains [string]$binding.authorizingScript) { throw 'DOGFOOD_V31_CORS_EXECUTION_BINDING_INVALID' }
$selfCim = Get-CimInstance Win32_Process -Filter "ProcessId=$PID" -ErrorAction Stop
if ([int]$selfCim.ParentProcessId -ne [int]$binding.authorizingStartPid) { throw 'DOGFOOD_V31_CORS_AUTHORIZING_PARENT_MISMATCH' }
...
if ([string]$contract.scripts.supervisor.sha256 -cne (Get-V31OriginSha256 $PSCommandPath) -or [string]$contract.scripts.sharedModule.sha256 -cne (Get-V31OriginSha256 $modulePath)) { throw 'DOGFOOD_V31_CORS_SCRIPT_HASH_MISMATCH' }

$api = if ($Mode -eq 'candidate') { $contract.candidateApi } else { $contract.rollbackApi }
$expectedState = Join-Path $repoRoot ([string]$api.statePath)
$expectedStop = Join-Path $repoRoot ([string]$api.stopPath)
if ((Get-V31OriginFullPath $StatePath) -cne (Get-V31OriginFullPath $expectedState) -or (Get-V31OriginFullPath $StopPath) -cne (Get-V31OriginFullPath $expectedStop)) { throw 'DOGFOOD_V31_CORS_RUNTIME_PATH_REJECTED' }
if ($Mode -eq 'candidate') {
  $preparedPath = Join-Path $repoRoot ([string]$contract.execution.preparedSourcePath)
  if (-not (Test-Path -LiteralPath $preparedPath) -or (Get-V31OriginSha256 $preparedPath) -cne [string]$binding.preparedSourceSha256) { throw 'DOGFOOD_V31_CORS_PREPARED_SOURCE_REJECTED' }
}
...
$environment = @{
  ...
  API_HOST = '127.0.0.1'
  API_PORT = '3303'
  WEB_ORIGIN = [string]$api.webOrigin
  PUBLIC_MCP_ORIGIN = [string]$api.publicMcpOrigin
...
while ($true) {
  ...
  try {
    ...
    $state = [ordered]@{artifactVersion=1;kind='DogfoodV31PublicMcpOriginCorsDecouplingApiSupervisor';status='RUNNING';role='api';mode=$Mode;supervisorPid=$PID;supervisorStartTimeUtc=$self.StartTime.ToUniversalTime().ToString('O');generation=$generation;restartCount=$restartCount;childPid=$child.Id;childStartTimeUtc=$childStart;listenerPid=$listenerPid;listenerStartTimeUtc=$listener.StartTime.ToUniversalTime().ToString('O');port=[int]$api.port;healthUrl=[string]$api.healthUrl;root=[string]$api.root;webOrigin=[string]$environment.WEB_ORIGIN;publicMcpOrigin=[string]$environment.PUBLIC_MCP_ORIGIN;coordinationFeatureEnabled=[bool]1;contractSha256=$ContractSha256;executionBindingSha256=$ExecutionBindingSha256;secretValuesSerialized=[bool]0;updatedAt=[DateTimeOffset]::UtcNow.ToString('O')}
  ...
  if (Get-NetTCPConnection -State Listen -LocalPort ([int]$api.port) -ErrorAction SilentlyContinue) { throw "DOGFOOD_V31_CORS_RESTART_PORT_BUSY:$($api.port)" }
  $restartCount++
