...
$runtimeResidue = @(Get-ChildItem -LiteralPath (Join-Path $runtimeRoot 'runtime') -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -like '*attempt-3*' })
if ($present.Count -ne 0 -or $runtimeResidue.Count -ne 0) { throw 'DOGFOOD_V31_FUTURE_OR_RUNTIME_RESIDUE' }
$result = [ordered]@{ artifactVersion = 1; kind = 'DogfoodV31RuntimeInvarianceAttempt3'; result = 'PASS'; listeners = $listeners; minioReady = $minio; activeBuildStatus = $activeBuild; candidateBuildStatus = $candidateBuild; futureArtifactsPresent = $present; candidateRuntimeResidue = @($runtimeResidue | ForEach-Object { $_.FullName }); capturedAt = [DateTimeOffset]::UtcNow.ToString('O') }
