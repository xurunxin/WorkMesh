...
foreach ($binding in @(@{ path = $modulePath; hash = $contract.scripts.module.sha256 }, @{ path = $supervisorPath; hash = $contract.scripts.supervisor.sha256 }, @{ path = $PSCommandPath; hash = $contract.scripts.rollback.sha256 }, @{ path = (Join-Path $runtimeRoot 'start-v31-stack.ps1'); hash = $contract.scripts.start.sha256 })) { if ((Get-V31Sha256 $binding.path) -cne [string]$binding.hash) { throw 'DOGFOOD_V31_SCRIPT_HASH_MISMATCH' } }
$protectedPids = @()
