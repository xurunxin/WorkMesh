...
$contract = [pscustomobject]@{ selectorBinding = 'v31-stack-activation-v1' }
$now = [DateTimeOffset]::Parse('2026-08-14T08:00:00Z')
$request = [pscustomobject]@{ selectorBinding = 'v31-stack-activation-v1'; scope = 'v31_stack_activation_once'; contractSha256 = ('a' * 64); gateSha256 = ('b' * 64); expiresAt = '2026-08-14T09:00:00Z' }
$approval = [pscustomobject]@{ selectorBinding = 'v31-stack-activation-v1'; scope = 'v31_stack_activation_once'; contractSha256 = ('a' * 64); gateSha256 = ('b' * 64); requestSha256 = ('c' * 64); decision = 'approved'; expiresAt = '2026-08-14T08:55:00Z' }
...
$result = [ordered]@{ artifactVersion = 1; kind = 'DogfoodV31ActivationContractProbe'; result = 'PASS'; gate = [ordered]@{ positive = [bool]1; negative = [bool]1 }; authorization = [ordered]@{ positive = 1; negative = $negativeCases }; compensation = $restoreMatrix; binaryTransformExact = $binaryTransformExact; directStageRejected = $directStageRejected; outsideRootRejectedPreWrite = $outsideRejected; capturedAt = [DateTimeOffset]::UtcNow.ToString('O') }
