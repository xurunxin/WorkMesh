...
function Start-Role([string]$Role, [string]$EffectiveContractSha, [string]$ExecutionBindingPath, [string]$ExecutionBindingSha) {
  $statePath = Join-Path $runtimeRoot "runtime\rollback-attempt-3-$Role.json"
  $stopPath = Join-Path $runtimeRoot "runtime\rollback-attempt-3-$Role.stop"
  Remove-Item -LiteralPath $statePath,$stopPath -Force -ErrorAction SilentlyContinue
  New-Item -ItemType Directory -Force -Path (Split-Path -Parent $statePath) | Out-Null
  $process = Start-Process -FilePath 'pwsh.exe' -ArgumentList @('-NoLogo','-NoProfile','-File',$supervisorPath,'-Role',$Role,'-Mode','rollback','-ContractPath',$contractPath,'-ContractSha256',$EffectiveContractSha,'-StatePath',$statePath,'-StopPath',$stopPath,'-ExecutionBindingPath',$ExecutionBindingPath,'-ExecutionBindingSha256',$ExecutionBindingSha) -WindowStyle Hidden -PassThru
...
foreach ($binding in @(@{ path = $modulePath; hash = $contract.scripts.module.sha256 }, @{ path = $supervisorPath; hash = $contract.scripts.supervisor.sha256 }, @{ path = $PSCommandPath; hash = $contract.scripts.rollback.sha256 }, @{ path = (Join-Path $runtimeRoot 'start-v31-stack-attempt-3.ps1'); hash = $contract.scripts.start.sha256 })) { if ((Get-V31Sha256 $binding.path) -cne [string]$binding.hash) { throw 'DOGFOOD_V31_SCRIPT_HASH_MISMATCH' } }
$protectedPids = @()
...
$executionBindingPath = Join-Path $runtimeRoot 'authorization\attempt-3-rollback-binding.json'
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $executionBindingPath) | Out-Null
$executionBinding = [ordered]@{ artifactVersion = 1; kind = 'DogfoodV31RollbackExecutionBinding'; contractSha256 = $actualContractSha; gateSha256 = $ContractGateSha256; requestSha256 = $RequestSha256; approvalSha256 = $ApprovalSha256; activationSha256 = $ActivationSha256; rollbackScriptSha256 = [string]$contract.scripts.rollback.sha256; allowedRoles = @('web','mcp'); allowedModes = @('rollback'); createdAt = [DateTimeOffset]::UtcNow.ToString('O'); expiresAt = ([DateTimeOffset]::UtcNow.AddMinutes(30)).ToString('O') }
...
foreach ($name in @('web','mcp')) {
  ...
  $null = Stop-V31ExactTree ([int]$runtime.supervisorPid) ([string]$runtime.supervisorStartTimeUtc) 'role-supervisor-attempt-3.ps1' ([string]$runtime.stopPath) $protectedPids "candidate-$name"
}
foreach ($name in @('web','mcp')) { $restored[$name] = Start-Role $name $actualContractSha $executionBindingPath $executionBindingSha }
