...
try {
  ...
  $operations = @{
    GetProcessRecord = { param([int]$ProcessId) Get-V31ProcessRecord $ProcessId }
    GetPortOwners = { param([int]$Port) @(Get-NetTCPConnection -State Listen -LocalPort $Port -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique) }
    AssertProcess = { param([int]$ProcessId,[string]$StartTimeUtc,[string[]]$CommandNeedles) Assert-V31Process $ProcessId $StartTimeUtc $CommandNeedles | Out-Null }
    GetListenerPid = { param([int]$Port) Get-V31ListenerPid $Port }
    WaitHttpReady = { param([string]$Url,[int]$Attempts) Wait-V31HttpReady $Url $Attempts }
    RemovePath = { param([string]$Path) Remove-Item -LiteralPath $Path -Force -ErrorAction SilentlyContinue }
    StartSupervisor = { param([string]$ScriptPath,[string[]]$Arguments,[string]$StatePath,[int]$Port,[string]$HealthUrl,[string]$Role) Start-V31Supervisor $ScriptPath $Arguments $StatePath $Port $HealthUrl $Role }
    StopCandidate = { param([string]$Role) Stop-CandidateIfPresent $contract $Role }
    AssertProtected = { Assert-Protected $contract }
    WriteReceipt = { param([object]$Receipt) Write-V31Json $rollbackPath $Receipt }
  }
  $compensation = Invoke-V31CoordinationCompensation $contract $controlRoot $ContractSha256 $primary $mutationStarted $operations
  throw "DOGFOOD_V31_COORD_ACTIVATION_FAILED:${primary}:$($compensation.result)"
}
